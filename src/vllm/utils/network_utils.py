...
def get_loopback_ip() -> str:
    loopback_ip = os.environ.get("VLLM_LOOPBACK_IP")
    if loopback_ip:
        return loopback_ip
...
def get_open_zmq_ipc_path() -> str:
    base_rpc_path = os.environ.get("VLLM_RPC_BASE_PATH", "/tmp")
...
def get_open_port() -> int:
    ...
    if "VLLM_DP_MASTER_PORT" in os.environ:
        dp_master_port = int(os.environ.get("VLLM_DP_MASTER_PORT", "0"))
...
def _get_open_port() -> int:
    port = int(os.environ.get("VLLM_PORT", "0")) if "VLLM_PORT" in os.environ else None
    if port is not None:
        while True:
            try:
